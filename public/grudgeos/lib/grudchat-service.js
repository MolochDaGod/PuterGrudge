class GrudChatService {
  ...
  async getSession() {
    try {
      return await this._kvGet("grudchat_session", null);
    } catch (e) {
      console.log("[GrudChat] Error getting session:", e.message);
      return null;
    }
  }
  ...
  async saveSession(userData) {
    try {
      ...
      await this._kvSet("grudchat_session", session);
      this.setUser(userData.userId || this.userId, userData.username);
      return session;
    } catch (e) {
      console.log("[GrudChat] Error saving session:", e.message);
      return null;
    }
  }
  ...
  async refreshSession() {
    try {
      ...
      if (session && session.username) {
        this.setUser(session.userId || this.userId, session.username);
        return session;
      }
    } catch (e) {
      console.log("[GrudChat] Error refreshing session:", e.message);
    }
    return null;
  }
  ...
  async clearSession() {
    try {
      await this._kvSet("grudchat_session", null);
    } catch (e) {
      console.log("[GrudChat] Error clearing session:", e.message);
    }
  }
}
